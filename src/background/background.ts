...
chrome.runtime.onInstalled.addListener(() => {
  // Set up initial chrome storage data.
  setStoredOptions({
    apiKey: '',
    enableNotifications: true,
  });
  setStoredRepositories([]);
  setStoredBadgeNumber(0);
  // Set alarm to update at period intervals.
  chrome.alarms.create({
    periodInMinutes: NOTIFICATION_INTERVAL_TIME,
  });
});

// For each alarm check: (1) update badge for all repositories
// and (2) Fire notitifications if there are any.
chrome.alarms.onAlarm.addListener(async () => {
  ...
  if (enableNotifications && notificationItems.length) {
    ...
    renderNotification(title, notificationItems, NOTIFICATION_CLEAR_TIME);
  }
});
